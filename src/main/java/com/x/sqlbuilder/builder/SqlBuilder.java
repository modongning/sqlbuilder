package com.x.sqlbuilder.builder;
...
import com.x.sqlbuilder.annotation.*;
import com.x.sqlbuilder.utils.FieldUtils;
...
public class SqlBuilder {
	...
	public SqlBuilder(Object obj, String orderBy, Integer firstResult, Integer maxResult) {
		super();
		this.obj = obj;
		this.orderBy = orderBy;
		this.firstResult = firstResult;
	...
	/**
	 * 创建查询sql 1.如没有设置排序方式，则默认使用倒序（DESC 2.默认排序为：
	...
	public String querySql() {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (null == fieldTransient)
				dbFields.append(columnName).append(" AS ").append(fieldName).append(",");

			// 默认ID排序
			if (0 >= orderByKeySize && null != dbId) {
			...
			if (null != objValue && !"".equals(objValue) && null == fieldTransient) {
				where.append(" AND ");
				if (null != dbColumn) {
					Like like = dbColumn.like();
					boolean likeValue = like.value();
					Like.LikeType likeType = like.type();
					if (likeValue) {
						where.append(columnName).append(" LIKE ?");
						if (Like.LikeType.PRE.equals(likeType)) {
							//前缀匹配
							allPrams.add("%" + objValue);
						} else if (Like.LikeType.ALL.equals(likeType)) {
							//全匹配
							allPrams.add("%" + objValue + "%");
						} else {
							//后缀匹配
	...
	public String countSql() {
		 ...
		 * 获取对象属性名和值
		 */
		String where = " WHERE 1=1" + createQueryWhereSql();
	...
	public String getTableName(Object obj) {
		...
		return null == dbTable ? obj.getClass().getSimpleName() : dbTable.value();
	}
	...
	private String getColumnName(Field field) {
		String fieldName = field.getName();
		DBColumn dbColumn = field.getAnnotation(DBColumn.class);
		return null == dbColumn ? fieldName : dbColumn.value();
	}
}
